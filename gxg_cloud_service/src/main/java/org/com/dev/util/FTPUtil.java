...
public class FTPUtil {
	...
	// 上传文件时，文件名称需要做编码转换 FTP协议里面，规定文件名编码为iso-8859-1 所以目录名或文件名需要转码
	private String fileCharset = "iso-8859-1";
		...
		try {
			...
			for (FTPFile ff : fs) {
				...
				if (ff.getName().equals(fileName)) {
					...
					ftpClient.retrieveFile(new String(ff.getName().getBytes(localCharset), fileCharset), is);
					is.flush();
					is.close();
	...
	public boolean deleteFile(String ftpDirAndFileName) {
		...
		try {
			flag = ftpClient.deleteFile(new String(ftpDirAndFileName.getBytes(localCharset), fileCharset));
		} catch (IOException e) {
			this.closeFTP();
			e.printStackTrace();
		}
	...
	public List<String> getFileListAll(String ftpPath) throws IOException {
		...
		System.out.println(ftpPath);
		// 获取指定文件下的所有文件名
		FTPFile[] files = ftpClient.listFiles(new String(ftpPath.getBytes(localCharset), fileCharset));
		...
		for (FTPFile f : files) {
			...
			if (f.isFile()) {
				...
				System.out.println(path);
				getFileListAll(new String(path.getBytes("GBK"), fileCharset));
			}
		}
		return fileList;
	}
		...
		try {
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (files[i].isFile()) {
					...
					fileList.add(new ZTreeNode(nodeId, pId, files[i].getName(),
							files[i].getName() + "--[" + fileSize + "]", ftpPath, files[i].getSize(), false));
					++nodeId;
				} else if (files[i].isDirectory()) {
					fileList.add(new ZTreeNode(nodeId, pId, files[i].getName(), files[i].getName(), ftpPath,
							files[i].getSize(), true));
					int pid = nodeId;
					nodeId++;
	...
	public FTPFile[] getFileList(String remotePath) {
		...
		try {
			...
			list = ftpClient.listFiles(new String(remotePath.getBytes(localCharset), fileCharset));
			return list;
		} catch (Exception e) {
	...
	public FTPFile[] getFtpServerFileList(String remotePath) {
		...
		try {
			FTPListParseEngine engine = ftpClient
					.initiateListParsing(new String(remotePath.getBytes(localCharset), fileCharset));
	...
	public FTPFile[] getFileListDirectories(String remotePath) {
		...
		try {
			...
			list = ftpClient.listDirectories(new String(remotePath.getBytes(localCharset), fileCharset));
			return list;
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			...
			if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode())) {
				...
				/*
				 ...
				 被动模式:就是指，一开始服务一起来，S端变开启一个端口告诉C端，我们之间的通讯就在这个端口下。也就C端被动的接受服务端。
				 */
					//设置被动模式(默认主动模式)
				   ...
				   // 设置传输模式  为被动传输模式
					...
					/*
					 * 要获取文件名之后，用name=new
					 * String(name.getBytes("UTF-8"),"GBK");这样的方式，当然并不一定就是这两种编码，
					 * 顺序也不一定. FileZilla
					 * Server默认使用utf-8，而我们的操作系统一般都是中文的，默认字符集是gbk，因此就不行了。
					 * 有建议使用FileZilla
					 * Server的某个特殊版或者打补丁，最简便的方法是给ftp服务器发送一个命令“opts utf8
					 * off”,就一切ok了。
					 */
					...
					ftpClientConfig.setServerLanguageCode("zh");
			 		System.out.println("Open FTP server Success:" + this.ftpIP + ";port:" + this.port + ";userName:"
							+ this.userName);
 		 			return true;
 		}
			} else {
